...
func (s *NamespacesStoreSuite) TestSACGetMany() {
	...
	ctxs := getSACContexts(objA, storage.Access_READ_ACCESS)
	for name, expected := range map[string][]*storage.NamespaceMetadata{
		withAllAccess:           []*storage.NamespaceMetadata{objA, objB},
		withNoAccess:            []*storage.NamespaceMetadata{},
		withNoAccessToCluster:   []*storage.NamespaceMetadata{},
		withAccessToDifferentNs: []*storage.NamespaceMetadata{},
		withAccess:              []*storage.NamespaceMetadata{objA},
		withAccessToCluster:     []*storage.NamespaceMetadata{objA},
	} {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
