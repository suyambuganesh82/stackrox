...
const (
	baseTable  = "networkpoliciesundodeployment"
	existsStmt = "SELECT EXISTS(SELECT 1 FROM networkpoliciesundodeployment WHERE DeploymentId = $1)"

	getStmt     = "SELECT serialized FROM networkpoliciesundodeployment WHERE DeploymentId = $1"
	deleteStmt  = "DELETE FROM networkpoliciesundodeployment WHERE DeploymentId = $1"
	walkStmt    = "SELECT serialized FROM networkpoliciesundodeployment"
	getIDsStmt  = "SELECT DeploymentId FROM networkpoliciesundodeployment"
	getManyStmt = "SELECT serialized FROM networkpoliciesundodeployment WHERE DeploymentId = ANY($1::text[])"

	deleteManyStmt = "DELETE FROM networkpoliciesundodeployment WHERE DeploymentId = ANY($1::text[])"
...
var (
	...
	schema = pkgSchema.NetworkpoliciesundodeploymentSchema
)
...
func New(ctx context.Context, db *pgxpool.Pool) Store {
	pgutils.CreateTable(ctx, db, pkgSchema.CreateTableNetworkpoliciesundodeploymentStmt)
...
func insertIntoNetworkpoliciesundodeployment(ctx context.Context, tx pgx.Tx, obj *storage.NetworkPolicyApplicationUndoDeploymentRecord) error {
	...
	finalStr := "INSERT INTO networkpoliciesundodeployment (DeploymentId, serialized) VALUES($1, $2) ON CONFLICT(DeploymentId) DO UPDATE SET DeploymentId = EXCLUDED.DeploymentId, serialized = EXCLUDED.serialized"
...
func (s *storeImpl) copyFromNetworkpoliciesundodeployment(ctx context.Context, tx pgx.Tx, objs ...*storage.NetworkPolicyApplicationUndoDeploymentRecord) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"networkpoliciesundodeployment"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.NetworkPolicyApplicationUndoDeploymentRecord) error {
	...
	if err := s.copyFromNetworkpoliciesundodeployment(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.NetworkPolicyApplicationUndoDeploymentRecord) error {
	...
	for _, obj := range objs {
		...
		if err := insertIntoNetworkpoliciesundodeployment(ctx, tx, obj); err != nil {
			if err := tx.Rollback(ctx); err != nil {
				return err
			}
			return err
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkPolicyApplicationUndoDeploymentRecord, []int, error) {
	...
	conn, release, err := s.acquireConn(ctx, ops.GetMany, "NetworkPolicyApplicationUndoDeploymentRecord")
	if err != nil {
		return nil, nil, err
	}
	defer release()
...
func (s *storeImpl) DeleteMany(ctx context.Context, ids []string) error {
	...
	conn, release, err := s.acquireConn(ctx, ops.RemoveMany, "NetworkPolicyApplicationUndoDeploymentRecord")
	if err != nil {
		return err
	}
	defer release()
...
func dropTableNetworkpoliciesundodeployment(ctx context.Context, db *pgxpool.Pool) {
	_, _ = db.Exec(ctx, "DROP TABLE IF EXISTS networkpoliciesundodeployment CASCADE")

}

func Destroy(ctx context.Context, db *pgxpool.Pool) {
	dropTableNetworkpoliciesundodeployment(ctx, db)
}
