...
func (s *RolebindingsStoreSuite) TestSACGetMany() {
	...
	ctxs := getSACContexts(objA, storage.Access_READ_ACCESS)
	for name, expected := range map[string][]*storage.K8SRoleBinding{
		withAllAccess:           []*storage.K8SRoleBinding{objA, objB},
		withNoAccess:            []*storage.K8SRoleBinding{},
		withNoAccessToCluster:   []*storage.K8SRoleBinding{},
		withAccessToDifferentNs: []*storage.K8SRoleBinding{},
		withAccess:              []*storage.K8SRoleBinding{objA},
		withAccessToCluster:     []*storage.K8SRoleBinding{objA},
	} {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
