...
// matchesAuth matches Docker integrations based on their authentication
// attributes.
...
// matchesAuthData matches ECR integrations based on their Authentication Data.
func matchesAuthData(this, other *storage.ImageIntegration) bool {
	thisAuth := this.GetEcr().GetAuthorizationData()
	otherAuth := other.GetEcr().GetAuthorizationData()
	return thisAuth == nil && otherAuth == nil ||
		thisAuth.GetUsername() == otherAuth.GetUsername() &&
			thisAuth.GetPassword() == otherAuth.GetPassword() &&
			thisAuth.GetExpiresAt().Equal(otherAuth.GetExpiresAt())
}

// matchingFunc returns true if two integrations match.
type matchingFunc func(*storage.ImageIntegration, *storage.ImageIntegration) bool
...
func (s *pipelineImpl) Run(ctx context.Context, clusterID string, msg *central.MsgFromSensor, _ common.MessageInjector) error {
	...
	// Set endpoint and matching function based on the integration type.

	var endpoint string
	var matches matchingFunc

	if dockerIntegration := imageIntegration.GetDocker(); dockerIntegration != nil {
		...
		endpoint = dockerIntegration.GetEndpoint()
		matches = matchesAuth
	} else if ecrIntegration := imageIntegration.GetEcr(); ecrIntegration != nil {
		endpoint = ecrIntegration.GetEndpoint()
		matches = matchesAuthData
	} else {
		return nil
	}

	imageIntegration.Name = fmt.Sprintf("Autogenerated %s for cluster %s", endpoint, clusterName)
