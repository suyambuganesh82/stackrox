...
if [[ -n "$CI" ]]; then
  ...
  kubectl -n stackrox patch deploy/sensor -p '{"spec":{"template":{"spec":{"containers":[{"name":"sensor","resources":{"requests":{"memory":"12Gi","cpu":"4"},"limits":{"memory":"12Gi","cpu":"4"}}}]}}}}'
  kubectl -n stackrox patch deploy/central -p '{"spec":{"template":{"spec":{"containers":[{"name":"central","resources":{"requests":{"memory":"12Gi","cpu":"4"},"limits":{"memory":"24Gi","cpu":"12"}}}]}}}}'
  if [[ "$ROX_POSTGRES_DATASTORE" == "true" ]]; then
    kubectl -n stackrox patch deploy/central-db -p '{"spec":{"template":{"spec":{"containers":[{"name":"central-db","resources":{"requests":{"memory":"12Gi","cpu":"4"},"limits":{"memory":"24Gi","cpu":"12"}}}]}}}}'
  fi
fi
